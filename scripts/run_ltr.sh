...
VLLM_LTR_DIR="${VLLM_LTR_DIR:-$EXPERIMENT_ROOT/vllm-ltr}"
...
python3 -m vllm.entrypoints.openai.api_server \
  ...
  --port "$PORT" > "$RESULT_DIR/server-ltr-$(date +%Y%m%d-%H%M%S).log" 2>&1 &
SERVER_PID=$!
...
for i in $(seq 1 120); do
  ...
  kill -0 $SERVER_PID 2>/dev/null || { echo "SERVER DIED, log tail:"; tail -30 "$RESULT_DIR"/server-ltr-*.log | tail -30; exit 1; }
  sleep 5
done
