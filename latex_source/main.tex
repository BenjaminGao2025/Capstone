...
Offered load determines whether this reordering is useful. When the queue is empty, OPT-125M inference adds overhead without creating a scheduling choice. Under load, several requests wait together and better ordering can reduce blocking, but a mistake also affects more requests sharing the KV cache and finite swap pool.
...
Attention & XFormers with \texttt{--enforce-eager}; no FlashAttention \\
...
V1 does not infer application criticality, and the aging gate should not be described as a substitute for an SLO. Aging guarantees that an old request eventually gains priority, regardless of its task, while an SLO-aware policy can promote a request before a deadline is violated \cite{huang2025slo}. A production design could combine the two signals, but it should keep their roles explicit: the deadline represents application urgency, whereas aging provides a general fairness bound when no deadline is available.
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.97\textwidth]{figures/github_latex_source.png}
\end{figure}
